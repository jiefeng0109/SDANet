...
class CtdetLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        opt = self.opt
        hm_loss, center_loss, all_loss, over_loss, wh_loss, off_loss, seg_loss, rnn_loss = 0, 0, 0, 0, 0, 0, 0, 0
        for s in range(opt.num_stacks):
            ...
            # rnn_loss = self.crit_rnn(output['forward'], output['backward']) / opt.num_stacks
        ...
        # loss = opt.hm_weight * hm_loss + opt.wh_weight * wh_loss + opt.off_weight * off_loss + opt.seg_weight * seg_loss
...
class CtdetTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        # loss_states = ['loss', 'hm_loss', 'wh_loss', 'off_loss', 'seg_loss']
